...
public class Textura2D implements Textura {
    ...
    private boolean reciclado; // MEJORA: Rastrear si el bitmap fue reciclado
    ...
    public Textura2D(Bitmap bitmap, float ancho, float alto) {
        ...
        // Obtener la configuración del bitmap
        Bitmap.Config config = bitmap.getConfig();

        // Asignar el formato de textura basado en la configuración
    ...
    public Textura2D(Textura textura, float ancho, float alto) {
        ...
        this.bitmap = Bitmap.createBitmap(
            textura.getBipmap(), 0, 0, (int) w, (int) h, max, false
        );
        new BitmapFactory.Options().inPreferredConfig = this.bitmap.getConfig();
    }
    ...
    @Override
    public void dispose() {
        // MEJORA: Verificar si ya está reciclado antes de reciclar
        if (bitmap != null && !reciclado && !bitmap.isRecycled()) {
            bitmap.recycle();
            reciclado = true;
        }
    }
    ...
    /**
     * MEJORA: Método para verificar si la textura está reciclada
     */
    public boolean isReciclada() {
        return reciclado || (bitmap != null && bitmap.isRecycled());
    }
}
