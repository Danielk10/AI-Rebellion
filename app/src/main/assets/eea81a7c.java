...
public abstract class Juego extends SurfaceView implements Runnable, SurfaceHolder.Callback {
    ...
    public Juego(AppCompatActivity actividad, ServicioBluetooth blueTooth) {
        ...
        // MEJORA: Inicializamos la cámara con el tamaño de la pantalla
    ...
    public void run() {
        ...
        while (iniciar) {
            ...
            // MEJORA: Verificar que el canvas no sea null
            pincel = holder.getSurface().lockHardwareCanvas();

            if (pincel == null) {
                continue; // Si el canvas es null, saltar este frame
            }
            ...
            try {
                ...
                pincel.drawRGB(
                        (Color.BLUE & 0xff0000) >> 16,
                        (Color.BLUE & 0xff00) >> 8,
                        (Color.BLUE & 0xff));

                // MEJORA: Aplicar transformación de cámara al buffer
                camara.aplicarTransformacion(pincelBufer.getCanvas());

                // Limpiar el buffer
                pincelBufer.limpiar(Color.BLUE);

                // Renderizar el juego con la cámara aplicada
                renderizar(pincelBufer, (float) delta);

                // Dibujar el buffer en el canvas principal
                pincel.drawBitmap(bufer.getBipmap(), null, rectangulo, null);

                // MEJORA: Resetear la transformación de la cámara para elementos de UI
                camara.resetearTransformacion(pincelBufer.getCanvas());

                // MEJORA: Dibujar FPS en coordenadas de pantalla (sin transformación de cámara)
                pincelBufer.getLapiz().setTextSize(18);
                pincelBufer.dibujarTexto(getFPS() + " FPS", 20, 20, Color.GREEN);

            } finally {
                // MEJORA: Siempre liberar el canvas, incluso si hay excepciones
